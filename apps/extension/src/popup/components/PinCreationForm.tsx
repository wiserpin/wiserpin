...
import { addPin, getSettings } from '@wiserpin/storage';
...
export function PinCreationForm({
  ...
  const [pendingCollectionSuggestion, setPendingCollectionSuggestion] = useState(false);

  // Track if auto-generation has already run to prevent re-running
  const hasAutoGeneratedRef = useRef(false);

  // Ref to the summary section for auto-scrolling
  ...
  const loadCurrentTab = async () => {
    ...
    try {
      ...
      if (tab?.url && tab?.title) {
        ...
        // Done loading metadata - UI is now interactive
        setLoadingMetadata(false);

        // Check settings for auto-generation (runs after UI is shown)
        const settings = await getSettings();

        // Only run auto-generation once, and only if we don't already have results
        if (!hasAutoGeneratedRef.current) {
          // Set pending state if both auto-generation tasks are enabled
          if (settings.autoSuggestCollection && aiAvailability === 'readily' && collections.length > 0) {
            setPendingCollectionSuggestion(true);
          }

          // Auto-generate summary first (collection suggestion needs it)
          let generatedSummary: string | null = null;
          if (settings.autoGenerateSummary && aiAvailability === 'readily' && !summary) {
            console.debug('[PinCreationForm] Auto-generating summary...');
            generatedSummary = await generateSummary(tab.url, tab.title);
          }

          // Auto-suggest collection after summary is ready, pass the summary directly
          if (settings.autoSuggestCollection && aiAvailability === 'readily' && collections.length > 0 && !selectedCollectionId) {
            console.debug('[PinCreationForm] Auto-suggesting collection with summary...');
            setPendingCollectionSuggestion(false); // No longer pending, now actively suggesting
            await suggestCollection(generatedSummary || undefined);
          } else {
            // Clear pending state if we skip collection suggestion
            setPendingCollectionSuggestion(false);
          }

          // Mark auto-generation as complete
  ...
  const generateSummary = async (pageUrl: string, pageTitle: string): Promise<string | null> => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Scroll to summary section after a brief delay to ensure content is rendered
        setTimeout(() => {
          if (summarySectionRef.current) {
            summarySectionRef.current.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
          }
        }, 100);

        return response.summary; // Return the summary for auto-suggestion
      } else {
  ...
  const suggestCollection = async (summaryText?: string) => {
    ...
    try {
      ...
      // Use provided summary or fall back to state summary
      ...
      const response = await chrome.tabs.sendMessage(tab.id, {
        ...
        collections: collections.map(c => ({
          id: c.id,
          name: c.name,
          goal: c.goal // Include goal for better AI matching
        }))
      });
  ...
  return (
    <>
      ...
      <div style={{ paddingBottom: '200px' }}>
          {/* Page Preview Card - Compact Horizontal Layout */}
          <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-white dark:bg-gray-800 shadow-sm" style={{ marginBottom: '24px' }}>
            <div className="flex gap-3 p-3">
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-center">
                ...
                <h3 className="font-semibold text-sm text-gray-900 dark:text-gray-100 mb-1 line-clamp-2" style={{ lineHeight: '1.4' }}>
                  {title || 'Untitled Page'}
                </h3>
          ...
          {/* AI Summary Section */}
          <div ref={summarySectionRef} style={{ marginBottom: '24px' }}>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium text-amber-600 dark:text-amber-400 mb-1">
                        {aiAvailability === 'readily' ? 'Error Generating Summary' : 'Chrome AI Not Available'}
                      </p>
          ...
          <>
            ...
            <div className="flex gap-2 mb-2">
              ...
              <button
                ...
                disabled={suggestingCollection || pendingCollectionSuggestion || collections.length === 0}
                ...
                title={pendingCollectionSuggestion ? "Waiting for summary..." : "AI Suggest Collection"}
              >
                {suggestingCollection || pendingCollectionSuggestion ? (
                  <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-indigo-600"></div>
                ) : (
